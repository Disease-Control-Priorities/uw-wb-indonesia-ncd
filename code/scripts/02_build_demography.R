...
#                  $base.pop [2 × 101]         population at 2025
...
for (i in seq_len(nrow(MODEL_COUNTRIES))) {
  ...
  p <- sf.wpp[[iso]]
  message(sprintf("    mx [%s] | mig [%s] | base.pop [%s] | Female mx(60, 2025) = %.4f",
...
for (iso in names(sf.wpp)) {
  ...
  stopifnot(
    ...
    "years start at 2025" = p$years[1]   == 2025L,
    "years end at 2155"   = p$years[131] == 2155L
  )
  if (any(is.na(p$mx[1:76, , ])))  warning(iso, ": NAs in mx (2025-2100)")
  if (any(is.na(p$base.pop)))       warning(iso, ": NAs in base.pop")

  total_pop <- sum(p$base.pop)
  message(sprintf("  %s \u2713  years %d\u2013%d | total pop 2025: %s",
                  iso, p$years[1], p$years[131],
                  formatC(round(total_pop), format = "d", big.mark = ",")))
  if (iso == "IDN" && (total_pop < 2e8 || total_pop > 3.5e8))
    warning("IDN total pop 2025 = ", round(total_pop / 1e6), "M — expected ~275M")
}

################################################################################
# 7  CROSS-CHECK: WPP(2025) vs GBD(2023) all-cause mx, ages 30-69
################################################################################

message("\n── Cross-check: WPP(2025) vs GBD(2023) all-cause mx, ages 30\u201369 ─────────")
