...
#   Health-outcome SOURCE OF TRUTH for Models 08 & 09 -- SUBNATIONAL edition.
#===========================================================================
# Copied verbatim from code/cvd-fair-choices/07_output_dalys.R and made
# province-aware with two localized changes (marked "SUBNATIONAL"):
#   * Disability weights (GBD 2023) are published only for Indonesia; the
#     national cause-level weights are BROADCAST to every modeled province.
#   * Life expectancy (WPP 2024) is published only for Indonesia; the national
#     single-year LE is BROADCAST to every modeled province (and to the saved
#     LE lookup Model 08 consumes).
# Both are documented limitations (no province-specific DW/LE inputs exist).
# Everything else -- the outcome contract, baseline pairing, and the CVD 40q30
# life-table -- is already keyed by `location` and works per province unchanged.
#
# --- Original production header -------------------------------------------------
...
#...........................................................
# SUBNATIONAL: province-specific disability weights / life expectancy do not
# exist. Broadcast the NATIONAL (GBD/WPP Indonesia) lookups to EVERY modeled
# location by joining on cause / (year, age) alone. Indonesia keeps its own real
# values; provinces inherit the national values (documented limitation).
dw_nat <- dw[location == "Indonesia", .(cause, disability_weight)]
if (!nrow(dw_nat)) dw_nat <- unique(dw[, .(cause, disability_weight)])
le_nat <- unique(le_lookup[location == "Indonesia", .(year, age, remaining_life_expectancy)])
if (!nrow(le_nat))
  stop("Model 07 (subnational): no national (Indonesia) life expectancy in the WPP lookup; ",
       "cannot broadcast LE to provinces.", call. = FALSE)
dt <- merge(dt, dw_nat, by = "cause",          all.x = TRUE)
dt <- merge(dt, le_nat, by = c("year", "age"), all.x = TRUE)
...
# SUBNATIONAL: broadcast the national single-year LE to every modeled location so
# Model 08 (which joins LE by location x year x age) finds LE for each province.
.model_locs   <- unique(dt$location)
le_lookup_out <- rbindlist(lapply(.model_locs, function(L) copy(le_nat)[, location := L]))
setcolorder(le_lookup_out, c("location", "year", "age", "remaining_life_expectancy"))
setkey(le_lookup_out, location, year, age)
# Broadcast disability weights to every modeled location too (national values).
dw_out <- rbindlist(lapply(.model_locs, function(L) copy(dw_nat)[, location := L]))
setcolorder(dw_out, c("location", "cause", "disability_weight"))
saveRDS(dt,            paste0(wd_outp, "dt_output_dalys.rds"))
saveRDS(le_lookup_out, paste0(wd_outp, "07_life_expectancy_lookup.rds"))
saveRDS(dw_out,        paste0(wd_outp, "07_disability_weights.rds"))
