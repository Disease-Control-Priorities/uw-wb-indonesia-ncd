...
#   VSL / VSLY monetary value of deaths averted -- SUBNATIONAL edition.
#===========================================================================
# Copied from code/cvd-fair-choices/08_economic_value_calculation.R and made
# province-aware with two localized changes (marked "SUBNATIONAL"):
#   * Every modeled location (38 provinces + the national reconciliation row) is
#     within Indonesia, so all take Indonesia's iso3 -> the SAME national per-
#     capita GNI / VSL / VSLY transfer parameters apply to every province
#     (documented national-valuation assumption; the reference case permits it).
#   * Province population (income denominator + average-adult-age for VSLY LE) is
#     taken from the MODEL's own baseline output, because provinces are absent
#     from the UN WPP national population file. Monetary value is thus
#     national-per-capita VSL/VSLY x PROVINCE-specific deaths averted / life-years
#     gained (both province-resolved from Models 06/07).
#
...
# SUBNATIONAL: every modeled location (38 provinces + the national row) is within
# Indonesia, so all take Indonesia's iso3 for the national GNI/VSL transfer
# (documented national-valuation assumption). Provinces are absent from the
# country-groupings file, so assign IDN directly rather than via a country join.
dt_compare[, iso3 := "IDN"]
dt_compare[, who_region := "South-East Asia"]
...
# SUBNATIONAL: province population is not in the UN WPP national file. Use the
# model's own BASELINE population (scenario-invariant reference = the province Nx
# that seeded the model) by taking `population` ONCE per location/year/age/sex
# (it is repeated across the per-cause rows) and summing sexes. The national row
# still equals its WPP-seeded population. Documented province approach.
.pop_src <- dt_h[scenario == base_id, .(pop_as = population[1]),
                 by = .(location, year, age, sex)]
dt_pop_unwpp <- .pop_src[year %in% model_years, .(Nx = sum(pop_as)), by = .(location, year, age)]
