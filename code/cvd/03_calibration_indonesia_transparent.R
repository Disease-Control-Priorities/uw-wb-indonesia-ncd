#===============================================================================
# 03_calibration_indonesia_transparent.R
#-------------------------------------------------------------------------------
# TRANSPARENT, SINGLE-PASS CALIBRATION FOR THE INDONESIA NCD / CVD MARKOV MODEL
#
# WHY THIS FILE EXISTS
# --------------------
# The production calibration is split across two opaque grid searches:
...
#                                   0-20% and CF by 0-50% (IRmult = 1-0.05*0:4,
#                                   CFmult = 1-0.05*0:10) and then renormalises
#                                   IR/CF against background mortality.
# Reviewers flagged the magic numbers: Why 5%? Why a 0-20% / 0-50% second pass?
# The two stages are also STACKED (031 multipliers, then 032 multipliers, then
# 05_build_baseline multiplies AGAIN with adjustments2023_age.csv), so the net
# adjustment applied to any cell is the product of three hidden factors.
#
# This script REPLACES the 031 + 032 split with ONE transparent search:
#   * one documented, SYMMETRIC multiplicative search range around 1
#     (SEARCH_HALFWIDTH below) for both IR and CF, instead of two ad-hoc grids;
#   * a per location-sex-cause iterative search (mirrors the cancer
#     calibrate_by_combo / calc_combo_error_fast template) that projects the
#     full well-sick-dead Markov trace and keeps the best-fitting multipliers;
#   * every tunable constant surfaced and commented in the PARAMETERS block
#     below -- nothing buried as a literal 0.05 / 0.20 / 0.50 inside a loop.
#
...
#                                  more flexible, use only if the primary fit is
#                                  clearly inadequate.
# Iteration 0 is always the baseline (all multipliers = 1), so the calibrated
# fit can never be WORSE than the uncalibrated baseline.
#
...
#   * calibration_factors_transparent.csv -- the calibrated per-combo (and per
#     age-group) IR/CF multipliers, analogous to adjustments2023_age.csv.
#   * calibration_diagnostics_transparent.csv -- baseline-vs-calibrated absolute
#     RMSE_deaths / RMSE_prev and weighted error per location-sex-cause-age.group.
#===============================================================================
...
PERTURB_SD       <- 0.12        # SD of Gaussian step applied to the best vector
RESTART_EVERY    <- 60          # every Nth iteration, draw a fresh uniform point
                                # in-bounds (escapes local minima); 0 disables
NO_IMPROVE_LIMIT <- 80          # stop a combo after this many non-improving iters
CONVERGE_TOL     <- 1e-4        # stop a combo once weighted error drops below this
...
## ----------------------------------------------------------------------------
## Reflect a numeric vector into [lo, hi] using triangle-wave folding, so a
## Gaussian step that overshoots the search bound bounces back in-range instead
## of being hard-clipped (clipping would pile probability mass on the bound).
## ----------------------------------------------------------------------------
reflect_bounds <- function(x, lo, hi) {
      rng <- hi - lo
      if (rng <= 0) return(rep(lo, length(x)))
      y <- (x - lo) %% (2 * rng)
      y[y < 0] <- y[y < 0] + 2 * rng
      y <- ifelse(y > rng, 2 * rng - y, y)
      lo + y
}
...
## ----------------------------------------------------------------------------
## Calibrate ONE combo: stochastic hill-climb (a (1+1) evolution strategy) over
## the multiplier vector. Iteration 0 = baseline (mult = 1). Each step perturbs
## the current best with a Gaussian, reflected into the search bounds; periodic
## uniform restarts escape local minima. Keeps the best-error vector found.
## ----------------------------------------------------------------------------
calibrate_one_combo <- function(combo_rates, pop_combo, combo_targets, age_match,
                                granularity, hw, n_iter, perturb_sd,
                                restart_every, no_improve_limit, converge_tol,
                                w_deaths, w_prev, eps, seed) {

      set.seed(seed)
      lo <- 1 - hw; hi <- 1 + hw
      age_groups <- sort(unique(combo_targets$age))
      n_par <- if (granularity == "age_group") 2L * length(age_groups) else 2L
      ...
      best_par <- rep(1, n_par)            # iteration 0: baseline
      ...
      for (it in 1:n_iter) {
            if (restart_every > 0 && it %% restart_every == 0) {
                  cand <- runif(n_par, lo, hi)                       # exploration
            } else {
                  cand <- reflect_bounds(best_par + rnorm(n_par, 0, perturb_sd), lo, hi)
            }
            err <- eval_par(cand); n_eval <- n_eval + 1L

            if (err < best_err) {
                  best_err <- err; best_par <- cand; no_improve <- 0L
            } else {
...
run_combo <- function(ci, combos, b_rates, pop20, targets, age_match) {
      ...
      fit <- calibrate_one_combo(cr, pc, ct, age_match,
                                 GRANULARITY, SEARCH_HALFWIDTH, N_ITER, PERTURB_SD,
                                 RESTART_EVERY, NO_IMPROVE_LIMIT, CONVERGE_TOL,
                                 W_DEATHS, W_PREV, EPS_REL, SEED + ci * 10000L)
...
cat(sprintf("Transparent calibration: %d location-sex-cause combos | granularity = %s\n",
            n_combos, GRANULARITY))
cat(sprintf("Search range per multiplier: [%.2f, %.2f] | up to %d iters/combo\n",
            1 - SEARCH_HALFWIDTH, 1 + SEARCH_HALFWIDTH, N_ITER))

worker_exports <- c(
      "combos", "b_rates", "pop20", "targets", "age_match",
      "make_age_match", "reflect_bounds", "enforce_tp_constraints", "build_mtab",
      "apply_multipliers", "project_combo", "proj_vs_targets", "combo_error",
      "combo_diag", "calibrate_one_combo", "run_combo",
      "GRANULARITY", "SEARCH_HALFWIDTH", "N_ITER", "PERTURB_SD", "RESTART_EVERY",
      "NO_IMPROVE_LIMIT", "CONVERGE_TOL", "W_DEATHS", "W_PREV", "EPS_REL",
...
fwrite(factors_out, paste0(wd_data, "calibration_factors_transparent.csv"))

## fit diagnostics: baseline-vs-calibrated absolute RMSE per combo x age.group
diag_full <- merge(diag_out, err_out[, .(location, sex, cause, base_err, cal_err,
                                         n_eval, n_par, hit_bound, bg_modified_rows)],
                   by = c("location", "sex", "cause"), all.x = TRUE)
fwrite(diag_full, paste0(wd_data, "calibration_diagnostics_transparent.csv"))
...
if (bg_rows_total > 0) {
      cat(sprintf("NOTE: BG.mx was renormalised (fallback) on %d rows where ",
          ...
          "in calibration_diagnostics_transparent.csv.\n", sep = "")
} else {
...
cat(sprintf("  %scalibration_factors_transparent.csv\n", wd_data))
cat(sprintf("  %scalibration_diagnostics_transparent.csv\n", wd_data))
